...
public class FinAsm extends Phase {
	...
	public void emit(
		...
		lines.clear();
		emitData(dataChunks, codeChunks);
	 ...
	 */
	private void emitData(final Vector<ASM.DataChunk> dataChunks, final Vector<ASM.CodeChunk> codeChunks) {
		...
		for (final ASM.CodeChunk codeChunk: codeChunks) {
			lines.add("_" + codeChunk.frame.label.name + ":");
			lines.add("    .dword 0");
		}
		lines.add("");
	}
	...
	private void emitCodeChunk(final ASM.CodeChunk codeChunk, final LinkedHashMap<MEM.Temp, String> registerMap) {
		...
		lines.add("la x30, _" + codeChunk.frame.label.name);
		lines.add("ld x31, 0(x30)");
		lines.add("addi x31, x31, 1");
		lines.add("sd, x31, 0(x30)");
		lines.add("addi x30, x0, 10");
		lines.add("ble x31, x30, _s_" + codeChunk.frame.label.name);
		lines.add("jal _exit");
		lines.add("_s_" + codeChunk.frame.label.name + ":");
